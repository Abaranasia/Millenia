...
Definition of done:
- [~] Feeding an impulse or short transient produces an audibly dense, smooth decaying tail with no crackling, denormal stalls, or per-block clicks. — **objective half now covered by `Tests/MilleniaTests` (`ScratchSchroederTankTests.cpp`)**: impulse response is finite, bounded, and decays past its peak to near-silence within 3.5s. The subjective half — does it actually *sound* dense and smooth, any audible crackle a numeric peak check wouldn't catch — still **needs a human ear pass**.
