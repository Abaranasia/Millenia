...
{
    auto estimate = [this] (const std::vector<float>& ring, std::array<double, lpcOrder + 1>& aOut, double& gainOut)
    {
        for (int i = 0; i < windowSamples; ++i)
        {
    ...
    estimate (dryRing, aDryIncoming, gainDryIncoming);
    estimate (shiftedRing, aShiftedIncoming, gainShiftedIncoming);
}
...
float FormantEnvelopeCorrector::processSample (float dry, float shifted) noexcept
{
    ...
    if (++hopCounter >= hopSamples)
    {
        ...
        // see quadratureCrossfadeSettled's declaration comment.
    ...
    if (crossfadeCounter >= crossfadeSamples)
    {
        ...
        // early-return check above with the wrong coefficients loaded.)
        //
        ...
        whiteningHistory = whiteningHistoryIncoming;
        resynthesisHistory = resynthesisHistoryIncoming;
        return incomingOutput; // == outgoingOutput + 1*(incomingOutput - outgoingOutput)
    }

    const float t = (float) crossfadeCounter / (float) crossfadeSamples;
    return outgoingOutput + t * (incomingOutput - outgoingOutput);
}
...
float FormantEnvelopeCorrector::processQuadratureSample (float shifted) noexcept
{
    ...
    // aDry/aShifted here already reflect whatever it just did THIS sample
    // -- including, if the ramp completed this very sample, promoting
    // aDry/aShifted to the former "incoming" coefficient values. The
    // quadrature-only history is NOT promoted by processSample() (it can't
    // be -- see that function's comment), so catch it up here exactly
    // once, on the first call that observes the ramp already complete.
    if (crossfadeCounter >= crossfadeSamples)
    {
        if (! quadratureCrossfadeSettled)
        {
            float finalOutput = applyTwoPassFilter (shifted, aShiftedIncoming, aDryIncoming, gainDryIncoming / gainShiftedIncoming, quadratureWhiteningHistoryIncoming, quadratureResynthesisHistoryIncoming);
            quadratureWhiteningHistory = quadratureWhiteningHistoryIncoming;
            quadratureResynthesisHistory = quadratureResynthesisHistoryIncoming;
            quadratureCrossfadeSettled = true;
            return finalOutput;
        ...
        return applyTwoPassFilter (shifted, aShifted, aDry, gainDry / gainShifted, quadratureWhiteningHistory, quadratureResynthesisHistory);
    }

    float outgoingOutput = applyTwoPassFilter (shifted, aShifted, aDry, gainDry / gainShifted, quadratureWhiteningHistory, quadratureResynthesisHistory);
    float incomingOutput = applyTwoPassFilter (shifted, aShiftedIncoming, aDryIncoming, gainDryIncoming / gainShiftedIncoming, quadratureWhiteningHistoryIncoming, quadratureResynthesisHistoryIncoming);
